...
@NgField(value = "readonly toastService = inject(WaToastDataService);", onSelf = false, onParent = true)
...
public class WaToastDataService<J extends WaToastDataService<J>> implements INgDataService<J> {
    ...
    public List<String> fields() {
        ...
        fields.add("readonly waToastService: WaToastService = inject(WaToastService);");
        return fields;
    }
    ...
    public List<String> methods() {
        ...
        list.add(
                "success(message: string, options: any = {}): string { return this.show(message, { variant: 'success', ...options }); }");
        list.add(
                "warning(message: string, options: any = {}): string { return this.show(message, { variant: 'warning', ...options }); }");
        list.add(
                "danger(message: string, options: any = {}): string { return this.show(message, { variant: 'danger', ...options }); }");
        list.add(
                "brand(message: string, options: any = {}): string { return this.show(message, { variant: 'brand', ...options }); }");
        list.add(
                "neutral(message: string, options: any = {}): string { return this.show(message, { variant: 'neutral', ...options }); }");
